            ...
            <div class="et_pb_column et_pb_column_2_3">
                ...
                <article id="post-<?php echo intval( $current_club['organizationId']); ?>"
                    ...
                    <div class="usc_clubs-entry-content">
                        ...
                        $html_string = '<div class="usc_clubs__contact clearfix">';
                        ...
                        if( !empty($current_club['email']) )
                            $emails_string .= '<p style="color: #555555;"><strong>General Club Contact</strong><br>'
                        ...
                        if( !empty($current_club['primaryContactName']) && !empty($current_club['primaryContactCampusEmail']) )
                            $emails_string .= '<p style="color: #555555;"><strong>Primary Club Contact</strong><br>' .  esc_html($current_club['primaryContactName']) . '<br>'
                        ...
                        if( !empty( $emails_string ) ) {
                            ...
                            $html_string .= '<div class="usc_clubs__contact__child usc_clubs__contact__emails">';

                            $html_string .= do_shortcode('[person title="Contact Information"]' . $emails_string . '[/person]');

                            $html_string .= '</div><!-- end of .usc_clubs__contact__emails -->';
                        }
                        ...
                        $social_links = array(
                            ...
                            'flickrFeedUrl'     => 'Flickr Feed',
                            'youtubeChannelUrl' => 'YouTube Channel',
                            'googleCalendarUrl' => 'Google Plus'
                        );

                        foreach($social_links as $key => $value) {
                            if( !empty( $current_club[$key]) )
                                $social_media_string .= '<a href="' . esc_url( $current_club[$key]) . '" target="_blank" class="social_media_links etmodules ' . $key . '"><span class="label">' . $value . '</span></a>' ;
                        }

                        //if there are no social media links, than just forget about this box altogether.
                        if( !empty( $social_media_string ) ) {

                            $html_string .= '<div class="usc_clubs__contact__child usc_clubs__contact__social_media">';

                            $html_string .= do_shortcode('[person title="Social Media Links"]' . $social_media_string . '[/person]');

                            $html_string .= '</div><!-- end of .usc_clubs__contact__social_media -->';

                        }

                        $html_string .= '<div class="usc_clubs__contact__child usc_clubs__contact__buttons"><div class="button_area_at_the_bottom_of_a_single_usc_club btn-menu">';
